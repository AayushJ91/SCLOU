...
def evaluate(
    ...
    for row in zone_readings:
        ...
        risk   = compute_risk_index(fos, rainfall, pressure, displacement)
        ...
        # ── Alert generation ───────────────────────────────────────────────
        if triggered:
            cause = f"Rainfall spike in {zid} ({rainfall:.1f} mm/h)"
            response = f"FoS dropped to {fos:.2f}; reinforcement {reinf.lower()}"
            outcome = f"Load redistributed and corridor stabilized at risk index {risk:.2f}"
            new_alerts.append({
                "zone": zid,
                "cause": cause,
                "response": response,
                "outcome": outcome,
                "message": f"{cause} → {response} → {outcome}",
                "timestamp": now,
            ...
            cause = f"Elevated rainfall and pore pressure in {zid}"
            response = f"Adaptive monitoring increased anchor tension to {anchor:.0f} kN"
            outcome = f"Zone remains watchlisted with risk index {risk:.2f}"
            new_alerts.append({
                "zone": zid,
                "cause": cause,
                "response": response,
